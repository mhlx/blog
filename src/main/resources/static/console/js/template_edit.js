(function() {
	...
	var fs = new FileSelector(function(datas) {
		if (datas.length > 0) {
			...
			for(const data of datas){
				...
				else if(["move","mp4"].includes(ext)){
					html += '<video src="'+rootPath+data.path+'"></video>';
				}
				else{
					html += '<a href="'+rootPath+data.path+'" target="_blank">'+data.name+'</a>';
				}
			}
	...
	function save(){
		...
		$.ajax({
			type : 'post',
			url : rootPath + 'console/templates/'+id+'/update',
