var fileChooser = (function() {
	...
	(function() {
		...
		createFolderModal
				...
				.click(
						function() {
							...
							var data = createFolderModal.find("form")
									.serializeObject();
							...
							var url = basePath + '/api/console/folder?path='
									+ data.path
							if (data.parent) {
		...
		fileSelectModal.find('[data-create-folder]').click(
				function() {
					fileSelectModal.modal('hide');
					createFolderModal.find("input[name='parent']").val(
							lastParam.parent);
	...
	(function() {
		...
		for (var i = 0; i < stores.length; i++) {
			uploadModal += '<option value="' + stores[i].id + '">'
					+ stores[i].name + '</option>';
		}
		uploadModal += '</select>';
		uploadModal += '</div>';
										...
										.each(
												...
												function(index, file) {
													var row = $('<tr class="template-upload" style="max-height:80px">'
															...
															+ '<td class="nowrap">'
															+ (!index
																	&& !o.options.autoUpload ? '  <button class="btn btn-primary start" disabled><i class="glyphicon glyphicon-upload"></i> <span>上传</span></button>'
																	: '')
		...
		uploadModal.find("form").attr(
				'action',
				root + 'api/console/store/'
						+ uploadModal.find('select[name="store"]').val()
						+ '/files')
	...
	var fileSelectPageQuery = function(page, parent) {
		...
		fileSelectMain
				.html("<img src='"
						+ basePath
						+ "/static/img/loading.gif' class='img-fluid mx-auto d-block' />")
		$
				.ajax({
					type : "get",
					...
					success : function(data) {
						...
						fileSelectMain.find('[data-toggle="tooltip"]')
								.tooltip();
					},
					error : function(jqXHR) {
						var message = $.parseJSON(jqXHR.responseText).error;
						fileSelectMain
								.html('<div class="row"><div class="col-md-12"><div class="alert alert-danger">'
										+ message + '</div></div></div>');
					},
				});
	...
	var getRenderHtml = function(data) {
		...
		if (paths.length > 0) {
			...
			for (var i = 0; i < paths.length; i++) {
				var path = paths[i];
				html += '<li class="breadcrumb-item"><a href="###" data-parent="'
						+ path.id
						+ '" data-page="1" >'
						+ path.path
						+ '</a></li>';
		...
		if (datas.length > 0) {
			...
			for (var i = 0; i < datas.length; i++) {
				...
				var style = isInSelected ? 'border:1px solid red;margin-bottom:10px;'
						: 'margin-bottom:10px;';
				html += '<div class="img-thumbnail text-center" style="'
						+ style + '">';
				if (data.type == 'DIRECTORY') {
					html += '<a href="###" data-page="1" data-parent="'
							+ data.id
							+ '" "><img src="'
							+ basePath
							+ '/static/img/folder.png" class="img-fluid" style="height:100px"/></a>';
				} else {
					var url = data.cf.url;
					var video = data.cf.extension.toUpperCase() == 'MP4'
							|| data.cf.extension.toUpperCase() == 'MOV';
					if (video) {
						html += '<a href="###" data-file="'
								+ data.id
					...
					if (data.cf.thumbnailUrl) {
						html += '<img  src="' + data.cf.thumbnailUrl.small
								+ '" data-middle="'
								+ data.cf.thumbnailUrl.middle
								+ '" class="img-fluid" style="height:100px"/>';
					} else {
						html += '<img src="'
								+ basePath
								+ '/static/img/file.png" class="img-fluid" style="height:100px"/>';
					}
					html += '</a>';
				}
				...
				html += '<a title="' + data.path + '" data-toggle="tooltip">'
						+ name + '</a>';
				html += '</div>';
				html += '</div>';
				html += '</div>';
		...
		if (page.totalPage > 1) {
			...
			for (var i = page.listbegin; i <= page.listend - 1; i++) {
				...
				html += '<a href="###" class="page-link" data-page="' + i
						+ '" data-parent="'
						+ (lastParam.parent ? lastParam.parent : "") + '" >'
						+ i + '</a>';
				html += '</li>';
			}
			html += '</ul>';
	...
	fileSelectModal.on("click", "[data-file]", function() {
		...
		for (var i = 0; i < datas.length; i++) {
			...
			if (data.id == id) {
				...
				// html += '<tr
				// data-selected="'+id+'"><td>'+data.path+'</td></tr>';
				// html += '</table>';
				// html += '</div>';
				// fileSelectedMain.html(html);
				// } else {
				// fileSelectedMain.find('table').append('<tr
				// data-selected="'+id+'"><td>'+data.path+'</td></tr>');
				// }
				// hasSelected = true;
				me.parent().css({
