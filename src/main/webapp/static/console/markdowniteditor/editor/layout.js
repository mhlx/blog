(function(config, editor) {
	...
	var toEditor = function() {
		...
		$("#toolbar")
				.append(
						'<i class="fas fa-eye icon" data-to-preview id="preview-icon" style="margin-right:0px" title="预览"></i>');
		if (editor) {
			editor.focus();
			if (_cursor) {
	...
	var toPreview = function() {
		...
		$("#toolbar")
				.append(
						'<i class="fas fa-eye-slash icon" span data-to-editor  id="editor-icon" style="margin-right:0px" title="取消预览"></i>');
	}
	...
	if (wwidth <= 768) {
		...
		$("head")
				.append(
						"<style type='text/css' id='mobile-style'>.icon {font-size: 30px} .CodeMirror-scroll{margin-top:10px} .inner-toolbar{width:100% !important;font-size:2.5rem !important;overflow-x:auto;white-space: nowrap} #search-box{width:100% !important}</style>");
	} else {
