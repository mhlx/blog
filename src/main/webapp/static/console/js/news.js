var table = datatable("newsTable", {
		url : function() {
			return root + 'api/console/newses';
		},
		paging:true,
		columns : [ {
			bind : 'allowComment',
			...
			render:function(v,d){
				var ele = $('<p>'+v+'</p>');
				var html = '<p>';
				html += '<p class="text-info">'+moment(d.write).format('YYYY-MM-DD HH:mm')+'</p>';
				html += '</p>';
				html += '<p>';
				html += ele.text();
				html += '</p>';
				var imgs = ele.find('img[src]');
				if(imgs.length > 0){
					html += '<div class="row">';
					$.each(imgs,function(i,v){
						var img = $(v);
						html += '<div class="col-md-4 col-6"><img src="'+img.attr('src')+'" class="img-thumbnail"></div>';
					})
					html += '</div>';
				}
				var videos = ele.find('video');
				if(videos.length > 0){
					html += '<p><small class="text text-warning">还有'+videos.length+'个视频未列出</small></p>';
				}
				return html;
			}
		}  ]
