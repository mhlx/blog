...
(function (global, factory) {
  ...
  // on a bidi boundary.
  // A cursor Pos(line, char, "before") is on the same visual line as `char -
	// 1`
  ...
  function scrollPosIntoView(cm, pos, end, margin) {
    ...
    if (!cm.options.lineWrapping && pos == end) {
      // Set pos and end to the cursor positions around the character pos
		// sticks to
      // If pos.sticky == "before", that is around pos.ch - 1, otherwise
		// around pos.ch
  ...
  function patchDisplay(cm, updateNumbersFrom, dims) {
    ...
    for (var i = 0; i < view.length; i++) {
      var lineView = view[i];
      if (lineView.hidden) ; else if (!lineView.node || lineView.node.parentNode != container) { // Not
																									// drawn
																									// yet
...
function onKeyPressMobile(e,cm) {
  ...
  if(!cm.display.input.composing) {
    cm.keyPressTimer = setTimeout( function () {
      onKeyPressOriginal.call(this$1, e ,cm);
    }, 30);
  }
}
