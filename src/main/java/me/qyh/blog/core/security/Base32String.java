...
public class Base32String {
	...
	public static byte[] decode(String encoded) throws SystemException {
		return getInstance().decodeInternal(encoded);
	}

	protected byte[] decodeInternal(String encoded) throws SystemException {
		...
		for (char c : encoded.toCharArray()) {
			if (!CHAR_MAP.containsKey(c)) {
				throw new SystemException("Illegal character: " + c);
			}
			buffer <<= SHIFT;
